"""Prepare the training, validation, and test sets for the MMOTU dataset. The dataset is already pre-split into training
and test data, ensuring that there is no patient overlap between these splits. However, no patient data is retained and
therefore we cannot ensure that there is not any patient overlap when splitting the training set into training and
validation data. We use a 8:2 split to separate the training and validation sets.
...
`validation.json`, or `test.json`). Each object has the following key/value pairs:
    ...
    - tumor_mask_multiclass: The path to the multi-class mask file (tumor segmentation by type).
    - scan_mask: The path to the scan mask file (scan segmentation).
    - label: The class label of the image.


Usage:
    python prepare_mmotu.py
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(prog="prepare_mmotu.py")
    parser.add_argument(
        "--dataset_dir",
        type=str,
...
def main():
    ...
    test_examples = process_examples(args.dataset_dir, output_dir, "test")
