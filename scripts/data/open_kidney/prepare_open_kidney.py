"""Divide the Open Kidney dataset into training, validation, and test splits using a 7:1:2 split. For the annotations,
use those created by Sonographer 1. Each image is associated with a distinct patient so we do not need to worry about
patient overlap between the splits. However, we do need to remove 20 duplicate images. The splits are stratified based
on the view label (longitudinal, transverse, other) to mitigate distribution shifts between the splits.
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(prog="prepare_open_kidney.py")
    parser.add_argument(
        "--dataset_dir",
        type=str,
