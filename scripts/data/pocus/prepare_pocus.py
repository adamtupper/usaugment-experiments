"""Build the POCUS image dataset by extracting the frames from the videos and copying the images. The dataset is split
into training, validation, and test sets using a 7:1:2 split.

Notes:
    - Following the same procedure as the authors of the original paper, we sample the videos at a rate of 3 Hz upto a
      maximum of 30 frames.
    - We include only convex probe ultrasound images in the dataset.
    - Following the same procedure as the authors of the original paper, we do not include the viral pneumonia class in
      the dataset due to the lack of examples.
    - The frames extracted from videos are grouped by the video source when splitting the dataset into training,
      validation, and test sets to prevent data leakage.
    - In total, there are 2102 examples split into 1468 training, 177 validation, and 457 test examples.
...
`validation.json`, or `test.json`). Each object has the following key/value pairs:
    ...
    - label: The pathology of the mass encoded as an integer (regular = 0, pneumonia = 1, covid = 2).

Usage:
    python prepare_pocus.py
...
def extract_frames_ffmpeg(video_path):
    ...
    print(VIDEO_INFO_TEMPLATE.format(os.path.basename(video_path), len(frames), frame_rate, frames[0].shape))
...
def main():
    ...
    examples = []
    image_dir = os.path.join(args.dataset_dir, INPUT_IMAGE_DIR)
    os.makedirs(os.path.join(output_dir, "images"), exist_ok=True)
    os.makedirs(os.path.join(output_dir, "masks", "scan"), exist_ok=True)
    for abbr, label in ABBR_TO_LABEL.items():
        files = [p for p in os.listdir(image_dir) if p.endswith(IMAGE_EXTENSIONS) and p.lower().startswith(abbr)]
        for file in files:
            # Convert any 1-channel (greyscale) or 4-channel (RGBA) images to 3-channel images
            ...
            skimage.io.imsave(os.path.join(output_dir, "images", f"{Path(file).stem}.png"), image)

            # Generate a scan mask for the image
            mask = generate_scan_mask(image)
            skimage.io.imsave(
                os.path.join(output_dir, "masks", "scan", f"{Path(file).stem}.png"), mask, check_contrast=False
            )
            ...
            examples.append(
                {
                    ...
                    "scan_mask": os.path.join("masks", "scan", f"{Path(file).stem}.png"),
                    "pathology": label,
    ...
    video_dir = os.path.join(args.dataset_dir, INPUT_VIDEO_DIR)
    for abbr, label in ABBR_TO_LABEL.items():
        files = [p for p in os.listdir(video_dir) if p.endswith(VIDEO_EXTENSIONS) and p.lower().startswith(abbr)]
        for file in files:
            if file.endswith(".gif"):
                frames, frame_rate = extract_frames_opencv(os.path.join(video_dir, file))
            else:
                # Process with imageio/ffmpeg
                frames, frame_rate = extract_frames_ffmpeg(os.path.join(video_dir, file))
            ...
            for i, frame in enumerate(frames):
                ...
                if frame.ndim == 2:
                    ...
                    image = (image * 255).astype(np.uint8)
                skimage.io.imsave(os.path.join(output_dir, "images", f"{Path(file).stem}_{i}.png"), frame)

                # Generate a scan mask for the frame
                mask = generate_scan_mask(frame)
                skimage.io.imsave(
                    os.path.join(output_dir, "masks", "scan", f"{Path(file).stem}_{i}.png"), mask, check_contrast=False
                )
                ...
                examples.append(
                    {
                        ...
                        "scan_mask": os.path.join("masks", "scan", f"{Path(file).stem}_{i}.png"),
                        "pathology": label,
                        "label": LABEL_TO_CLASS[label],
                    }
                )
    ...
    train_val_indices, test_indices = next(splitter.split(X=examples, groups=examples["source"]))
    train_val_examples = examples.iloc[train_val_indices]
    test_examples = examples.iloc[test_indices]

    splitter = GroupShuffleSplit(n_splits=1, test_size=0.1, random_state=42)
    train_indices, val_indices = next(splitter.split(X=train_val_examples, groups=train_val_examples["source"]))
