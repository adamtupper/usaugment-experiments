...
class ClassificationDataset(JSON_Dataset):
    def __init__(
        self, root_dir, split, transform=None, image_key="image", label_key="label", scan_mask_key="scan_mask"
    ):
        """
        Arguments:
            root_dir (string): Directory with all the images.
    ...
    def __getitem__(self, idx):
        ...
        scan_mask_path = os.path.join(self.root_dir, self.metadata[idx][self.scan_mask_key])
...
class DetectionDataset(JSON_Dataset):
    def __init__(
        self, root_dir, split, transform=None, image_key="image", bbox_key="bboxes", labels_key="class_labels"
    ):
        """
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.root_dir, self.metadata[idx][self.image_key])
        image = io.imread(image_path)
        bboxes = self.metadata[idx][self.bbox_key]
        bbox_labels = self.metadata[idx][self.labels_key]
...
class SegmentationDataset(JSON_Dataset):
    def __init__(self, root_dir, split, transform=None, image_key="image", mask_key="mask", scan_mask_key="scan_mask"):
        """
        Arguments:
            root_dir (string): Directory with all the images.
            split (string): The split to use, i.e. "train", "validation" or "test".
